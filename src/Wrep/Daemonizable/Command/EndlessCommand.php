...
abstract class EndlessCommand extends Command
{
     ...
     * @return int
     */
    protected function parseInterval($interval)
    {
        preg_match(
            '/^(?:(\d+)h)?(?:(\d+)m)?(?:(\d+)s)?$/i',
            $interval,
        ...
        if (isset($matches[1]) && $matches[1] != '') {
            $timeout += 60 * 60 * $matches[1];
        }

        if (isset($matches[2]) && $matches[2] != '') {
            $timeout += 60 * $matches[2];
        }
    ...
    {
        $lastUsage                 = ($peak) ? $this->lastPeakUsage
            : $this->lastUsage;
        $info['amount']            = ($peak) ? memory_get_peak_usage()
            : memory_get_usage();
        $info['diff']              = $info['amount'] - $lastUsage;
        $info['diffPercentage']    = ($lastUsage == 0) ? 0
            : $info['diff'] / ($lastUsage / 100);
     ...
     * Execution logic.
     *
     * This method will be called on every iteration. Try to keep it fast,
     * process only one unit every iteration. If one unit is to inefficient
     * (due networking for example), process small batches and call the
     * throwExceptionOnShutdown whenever you can. This prevents unexpected
     * kills of the process and makes shutdown fast.
     *
     * @param InputInterface  $input  An InputInterface instance
     * @param OutputInterface $output An OutputInterface instance
     *
    ...
    public function setReturnCode($returnCode)
    {
        if ($returnCode < 0) {
            throw new \InvalidArgumentException(
                'Invalid returnCode provided to Command::setReturnCode.'
            );
        }
     ...
     *
     * Can be used to (voluntary) exit the runloop during a run, use this if
     * your execution code takes quite long to finish on a point where you
     * still can exit without corrupting any data.
     *
     * @return Command The current instance
     *
     ...
     *
     * Use this to do some cleanup, but keep it fast. If you take too long and
     * we must exit because of a signal changes are the process will be killed!
     * It's the counterpart of initialize().
     *
     * @param InputInterface  $input  An InputInterface instance
     * @param OutputInterface $output An OutputInterface instance, will be a
     *                                NullOutput if the verbose is not set
     */
